% This program demenstrates how to redatum the DAS VSP data to virtual SSP
% data by interferometry method.
%
...
nsrc = 100;                 % nsrc--number of sources
dsrc = 1;                   % dsrc--sources interval in grid points;
recordVideoFlag = 1;
...
%% Velocity model
% Model dimensions
nx = 170;                   % numbers of x coordinate index
nz = 200;                   % numbers of z coordinate index
...
% Generate wave data with reflections from free surface in heter model
opts.fsFlag = 1;
tic;
parfor i=1:nsrc
    % place source
...
for new_sou = round(abs_thick/upscale):nrec-round(abs_thick/upscale)
    ...
    for new_rec = round(abs_thick/upscale):nrec-round(abs_thick/upscale)
